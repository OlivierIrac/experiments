...
class WindowDecoration:
    ...
    def draw(self, surface):
        pygame.draw.rect(surface, WindowDecoration.COLOR, pygame.Rect(self.myWindow.left - WindowDecoration.BORDER_THICKNESS, self.myWindow.top - WindowDecoration.BORDER_THICKNESS,
                                                                      self.myWindow.width + WindowDecoration.BORDER_THICKNESS, self.myWindow.height + WindowDecoration.BORDER_THICKNESS), WindowDecoration.BORDER_THICKNESS)
        pygame.draw.rect(surface, WindowDecoration.COLOR, pygame.Rect(self.myWindow.left - WindowDecoration.BORDER_THICKNESS, self.myWindow.top -
                                                                      WindowDecoration.TITLE_HEIGHT, self.myWindow.width + 2 * WindowDecoration.BORDER_THICKNESS, WindowDecoration.TITLE_HEIGHT))
        blit_text(surface, self.myWindow.title, (self.myWindow.left, self.myWindow.top - WindowDecoration.TITLE_FONT_SIZE -
                                                 (WindowDecoration.TITLE_HEIGHT - WindowDecoration.TITLE_FONT_SIZE) / 2), self.titleFont, WindowDecoration.TITLE_COLOR)
    ...
    def handleWindows(self, event):
        ...
        for window in self.windowsList:
            ...
            if (event.type == pygame.MOUSEBUTTONDOWN and not self.windowSelected):
                if (event.button == 1 and window.resize is False and window.drag is False):
                    ...
                    if (not window.decoration and window.left <= mx < window.left + window.width - 2 and window.top <= my < window.top + window.height - 2):
                        self.selectWindow(window)
                        self.drag = True
                ...
                if (window.left <= mx < window.left + window.width and window.top <= my < window.top + window.height and not zoom):
                    if (event.button == 4):
                        (window.width, window.height) = resizeProportional(window.width, window.height,
                                                                           window.width + 2 * WindowManager.ZOOM_SPEED, window.height + 2 * WindowManager.ZOOM_SPEED)
