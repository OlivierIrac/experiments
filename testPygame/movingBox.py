...
class MyWindow: 
    ...
    def draw():
        ...
        for window in MyWindow.windowsList:
            print ("window.x0 %i,window.y0 %i,window.height %i,window.width %i"%(window.x0,window.y0,window.height,window.width))
            window.drawFunction(window.x0,window.y0,window.height,window.width)
        
    def __init__ (self, drawFunction, windowWidth=800, windowHeight=600, xLeft = 0, yTop = 0):
...
#        print ("mx:%i, my:%i, self.x0:%i, self.width:%i"%(x,y,self.x0,self.width))
#        print ("test corner", ((self.x0 + self.width -2) <= x <= (self.x0 + self.width + 5) and (self.y0 + self.height -2) <= y <= (self.y0 + self.height + 5)))
    ...
    def handleWindows():
        ...
        for window in MyWindow.windowsList:
            ...
            if (window.drag == True):
                window.x0 = window.x00 + mx - mxPress
                window.y0 = window.y00 + my - myPress
                
            if (window.resize == True):
                window.width = window.width0 + mx - mxPress
                window.height = window.height0 + my - myPress
...
olivierFaceWindow=MyWindow(olivierFaceDraw)

while (not done):
    pygame.display.set_caption(" FPS : {:.4}".format(clock.get_fps()))
    
    MyWindow.handleWindows()
