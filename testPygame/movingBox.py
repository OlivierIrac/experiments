    ...
    def draw(self):
        ...
        for window in reversed(self.windowsList):
            ...
            if (window.decoration):
                pygame.draw.rect(screen, WindowManager.WINDOW_DECORATION_COLOR, pygame.Rect(
                    window.left, window.top, window.width, window.height), 2)
        ...
        for window in self.windowsList:
            ...
            if (event.type == pygame.MOUSEBUTTONDOWN and not self.windowSelected):
                if (event.button == 1 and window.resize is False and window.drag is False):
                    ...
                    if (window.left <= mx < window.left + window.width - 2 and window.top <= my < window.top + window.height - 2):
                        self.selectWindow(window)
                        self.drag = True
...
class ImageHandler(WindowHandler):
    def __init__(self, file, left=0, top=0, width=800, height=600, decoration=False, proportional=False):
        WindowHandler.__init__(self, pygame.image.load(file), left, top,
                               width, height, decoration, proportional)
...
img1Window = ImageHandler(os.path.join(
    'data', '2015_06_27_EOS 70D_0978.jpg'), 50, 50, 100, 100, True, True)
